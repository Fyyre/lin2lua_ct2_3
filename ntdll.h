/*
	ntdll.h
	User Mode, X86 & X64 compatible version
	Visual Studio 6.0 - Visual Studio 2010
	Intel C++ Compiler (ICL) 11.x - 12.x

	(c) 2009, 2010, 2011 - Fyyre
	(c) 2011 - EP_X0FF
	(c) 2011 - rndbit

	version 1.20 ( increment this if changes has global effect )
	please mark your changes with comment and date

	last change 31/05/2011

	todo: switch use of WIN64/WIN32 to _M_X64/_M_X86 - make sure does not break things.
*/
...
//
// Data Types -- DOT NOT modify -- modification will break _WIN32 & _WIN64 compatibly.
//
...
#if defined(_WINNT_) && (_MSC_VER < 1500)
...
__inline struct _TEB * NtCurrentTeb() { return (struct _TEB *) (ULONG_PTR) __readfsdword (PcTeb); }
...
#if !defined(_WINNT_)
#define _WINNT_		// enforce presidence over winnt.h
...
#if defined(_WINNT_) && (_MSC_VER >= 1500) || !defined(_WINNT_) && (_MSC_VER == 1400)
...
typedef struct _XSTATE_CONFIGURATION {
	DWORD64 EnabledFeatures;	// Mask of enabled features
	DWORD Size;	// Total size of the save area
	DWORD OptimizedSave : 1;
	XSTATE_FEATURE Features[MAXIMUM_XSTATE_FEATURES];	// List of features
} XSTATE_CONFIGURATION, *PXSTATE_CONFIGURATION;
